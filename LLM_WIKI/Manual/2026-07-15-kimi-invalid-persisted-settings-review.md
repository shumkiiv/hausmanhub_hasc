...
The HausmanHub outer Home Assistant adapter, its isolated Core lifecycle check, local
...
entity while it starts, even when HausmanHub rejects invalid saved settings. HausmanHub now
waits until startup has finished, then removes only state entries belonging to
that invalid HausmanHub config entry.

- An invalid saved setup rejects an immediate reload and its local page returns
  only unavailable, without counts.
- After a full empty restart, it cannot restore HausmanHub runtime data, services,
  devices, local page, or count states.
- The cleanup is limited to HausmanHub's own registered state IDs. It does not touch
