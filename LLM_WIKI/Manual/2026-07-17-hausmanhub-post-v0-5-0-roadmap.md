# HausmanHub roadmap after 0.5.0

Date: 2026-07-17.

## Product direction

HausmanHub is the platform that owns autonomous home-control contours. Climate is
...
parameters and safety limits, then let HausmanHub run it automatically. Android is a
status, setup, and temporary-override client of HausmanHub, not the place where
automation rules execute.

Rooms, device roles, targets, lifecycle, and operation move behind HausmanHub. The
...
path. Android talks only to HausmanHub and does not know which engine implements a
contour.

This roadmap covers HausmanHub source only. Installing it in a live home, preparing a
...
`disabled`, prepare a valid registry through HausmanHub's own UI, and see a redacted
...
1. Generate HausmanHub-side operation IDs and return typed receipts instead of only
   `submitted` or `shadow`.
...
3. Observe the existing climate-core result and physical feedback; do not let
   HausmanHub infer success from HTTP acceptance alone.
...
2. Add capability-driven Android screens only after the HausmanHub public schema and
   command lifecycle are stable. Android work stays in its own repository and
   authorization scope.
3. Migrate the tablet to HausmanHub-only room/device IDs; raw HA entities and
...
## Phase 6 — native HausmanHub climate controller

1. Store a HausmanHub-owned one-room comfort policy and calculate temperature and
...
4. Expose HausmanHub-owned room targets and decisions to Android through a versioned
   public contract, preserving the current stable HausmanHub room/device IDs.
...
atomically. The public `hausman-hub-contours` v1 contract contains no private
source/entity IDs. Automatic status requires fresh state, engine auto mode,
authority, device availability, and matching targets. HausmanHub sends no climate
